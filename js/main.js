function rango(primerAnio,segundoAnio){
 ...
 /*   var select = document.getElementById("exampleFormControlSelect3"); //Seleccionamos el select
   ...
   var select = document.getElementById("exampleFormControlSelect1"); //Seleccionamos el select
      ...
      parrafo.parentNode.removeChild();
 
///////////// funcion para tomar modelos y cargar anios
   
///////////// evento onchange para cambiar marca y modelo
   ...
   function buscarModelos(){
      ...
      switch (buscarMarca){
         case ("PEUGEOT"):
            ...
            var option = document.createElement("option"); //Creamos la opcion
            option.innerHTML = BaseDatosAuto[0].modelo[i].modelo; //Metemos el texto en la opción
            select.appendChild(option); //Metemos la opción en el select
         ...
         case ("CHEVROLET"):
            ...
            for(var i=0; i < BaseDatosAuto[1].modelo.length; i++){ 
            var option = document.createElement("option"); //Creamos la opcion
            option.innerHTML = BaseDatosAuto[1].modelo[i].modelo; //Metemos el texto en la opción
            select.appendChild(option); //Metemos la opción en el select
         }
         break;
      }
